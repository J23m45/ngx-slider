  ...
  implements OnInit, AfterViewInit, OnChanges, OnDestroy, ControlValueAccessor
{
  ...
  lines: { leftOffset: string; width: string }[];
  ...
  getLineStyle(
    ...
    const leftOffset =
      ((startValue - this.options.floor) / totalRange) * 100 + offset;
    const width =
      ((endValue - startValue) / totalRange) * 100 + widthAdjustment;
  ...
  getLineWidth(startTick: any, endTick: any): string {
    if (!endTick) {
      return "0%"; // No end tick means no line should be drawn.
    }
    // Calculate total range from slider options.
    const totalRange = this.options.ceil - this.options.floor;
    // Calculate width as a percentage of the slider's total width.
    const widthPercentage =
      ((endTick.value - startTick.value) / totalRange) * 100;
